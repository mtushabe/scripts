...
dreams=`aws s3 ls s3://34c8027a.eu-central-1.mets.bao.systems/backups/ --recursive | sort | tail -n 1 | awk '{print $4}'`

echo $dreams

aws s3 cp s3://34c8027a.eu-central-1.mets.bao.systems/$dreams /home/mtushabe/dreamsbackup

latest=$(ls -t /home/mtushabe/dreamsbackup/*.gz | head -n1)
echo "$latest"
gunzip $latest

newsql=$(ls -t /home/mtushabe/dreamsbackup/*.sql | head -n1)
echo "$newsql"
...
WHERE pg_stat_activity.datname = 'dreams182'
  AND pid <> pg_backend_pid();

DROP DATABASE dreams182; --drop the DB

CREATE DATABASE dreams182;
...
PGPASSWORD=_Postgres12_ psql -U postgres -p 5432 -h localhost -d dreams182 -f $newsql
